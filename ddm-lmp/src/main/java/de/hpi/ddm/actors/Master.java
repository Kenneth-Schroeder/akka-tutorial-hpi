...
public class Master extends AbstractLoggingActor {
    ...
    ////////////////////
    @Data
    public static class EndMessage implements Serializable { // INFO: good practice that actor defines his messages as 'interface description'
    ...
    private final BloomFilter data = new BloomFilter(BloomFilter.DEFAULT_SIZE, true); // INFO: large message content
    ...
    protected void handle(RegistrationMessage message) {
        ...
        this.context().watch(this.sender()); // INFO: start watching workers that have registered and add them to the workers list
        this.workers.add(this.sender());
        this.log().info("Registered {}", this.sender());

        // INFO: send the large message to the registered worker over a PROXY, which is created just for that
